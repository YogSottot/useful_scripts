...
crontab -l | { cat; echo "30 1 * * * /opt/backup/restic/restic-wrapper.sh $1 $3 backup --exclude-file=/opt/backup/restic/exclude/$1.txt $2 > /dev/null 2>&1 && /opt/backup/restic/restic-wrapper.sh $1 $3 diff \$(echo \`/usr/local/bin/restic snapshots --json | /usr/bin/jq -r '.[-2:][].id'\`) > /dev/null 2>&1 || true"; } | crontab -
