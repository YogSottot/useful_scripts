<?php
/*
 * Скрипт бэкапит только БД, без публичной части сайта, без Ядра, без статистики, без символьных ссылок
 *
 * Задача для cron
 * /usr/bin/php -f /opt/backup/backup_bitrixdb.sh /путь.к.корню.сайта
 *
 * Настройки этого бэкапа заданы вручную  в массиве $arExpertBackupDefaultParams, т.к. отличаются от полного бэкапа осн. сайта
 *
*/
...
else
{
	...
	if (!$_SERVER['DOCUMENT_ROOT'])
		$DOCUMENT_ROOT = $_SERVER['DOCUMENT_ROOT'] = ($argv[1]);
...
else
{
	...
	$arExpertBackupDefaultParams = array(
		'dump_base' => 1, //включаем бэкап БД
		'dump_base_skip_stat' => 1, //отключаем статистику
		'dump_base_skip_search' => 1, //отключаем поисковый индекс
		'dump_base_skip_log' => 1, //отключаем журнал событий

		'dump_file_public' => 0, //отключаем публичную часть
		'dump_file_kernel' => 0, //отключаем архивацию ядра
		'dump_do_clouds' => 0, //отключаем бэкап файлов в облаке
