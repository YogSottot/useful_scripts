...
nice -n 19 ionice -c2 -n7 tar cfp - --exclude=www/bitrix/tmp/* --exclude=www/bitrix/updates/* --exclude=www/bitrix/backup/* --exclude=www/bitrix/*cache/* --exclude=www/bitrix/html_pages/* --exclude=www/upload/* -C /home/bitrix/ www/ | gzip -c > ${backup_dir}/${name}.tar.gz && nice -n 19 ionice -c2 -n7 /root/.local/bin/swift -q -A https://auth.selcdn.ru -U ${login} -K ${userkey} upload -H "X-Delete-After: 259200" --object-name `date +%Y-%m-%d-%H:%M`/ ${storage_dir} ${backup_dir}/ ; rm -rf ${backup_dir}/* && echo OK && exit
#echo Error
